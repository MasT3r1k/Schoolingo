...
export class Settings {
    ...
    public refreshSecurityAPI(): void {
        this.http.get<SecurityAPI | { error: string }>(Config.API_URL + "/v1/security", { withCredentials: true })
        .subscribe((data) => {
            if ('error' in data) {
    ...
    public getBackupCodes(): string[] {
        ...
        this.http.post(Config.API_URL + "/v1/security", { method: "GET_BACKUP_CODES", TFA: this.TFAControl.value }, { withCredentials: true })
        .subscribe((data) => {
            console.log(data);
        });
        return [];
