...
export class DatalistComponent implements OnInit {
    ...
    public getMaxPages(): number {
        return Math.ceil((this.metadata.rows || 0) / this.dataPerPage);
    }

    public refreshData(): void {
        this.visibleData = this.data.getValue().slice(0, this.page.getValue() * this.dataPerPage);
    }

    public loadData(): void {
        if (!this.options.url) return;
        this.socketService.emit(this.options.url, { limit: this.dataPerPage, page: this.page.getValue(), ignore: this.options.ignore ?? [], search: this.search.value ?? "" });   
    }
