...
export class Schoolingo {
  ...
  public getRefreshingText(): string {
    ...
    if (this.wifiConnection == false) {
      return (
        "<i class='ti ti-x'></i> " +
        this.locale.getLocale('buttons/noInternetConnection')
      );
    }
    if (this.tempData['refreshingConnection'] == true) {
      return (
        "<div class='btn-loader mr'></div>" +
        this.locale.getLocale('buttons/updating')
      );
    }
    if (this.tempData['refreshingConnection'] == 'error') {
      return (
        "<i class='ti ti-x'></i> " + this.locale.getLocale('buttons/failed')
      );
    }
  ...
  public setupSocket(): void {
    this.socketService.addFunction('token', (data: any) => {
      ...
      if (data.status === 1) {
        ...
        this.userService.setExpiration(data.user.expires);
        console.log(data);
        // this.userService.setToken(data.user.token, );
        this.boardSidebar = this.getBoardSidebar();

        let expireTime =
          new Date(data.user.expires).getTime() - new Date().getTime();
        setTimeout(() => {
          this.socketService.getSocket().Socket?.disconnect();
          this.socketService.connectAnon();
          this.SECURITY_modal = 'autologout';
    ...
    this.socketService.addFunction('timetable', (timetable: any[]) => {
      ...
      for (let i = 0; i < timetable.length; i++) {
        ...
        while (
          timetable[i].hour != 0 &&
          lessons[timetable[i].day] &&
          timetable[i].hour - 1 - d1 >= 0 &&
        ...
        if (
          lessons?.[timetable[i].day]?.[timetable[i].hour - 1]?.[0]?.subject ==
            -1 &&
          lessons?.[timetable[i].day]?.[timetable[i].hour - 1]?.[0]?.teacher ==
            -1
        ) {
          lessons[timetable[i].day][timetable[i].hour - 1] = [];
        }
  ...
  public checkPermissions(required: UserPermissions[]): boolean {
    ...
    if (
      !user &&
      (required.includes('onlynonlogged') || required.includes('nonlogged'))
    )
      return true;
    ...
    return (
      ...
      (required.includes('classteacher') &&
        user.type == 'teacher' &&
  ...
  public getBoardSidebar(): SidebarGroup[] {
    ...
    boardSidebar.forEach((section: SidebarGroup) => {
      if (section.permission && !this.checkPermissions(section.permission))
        return;
      ...
      section.items.forEach((item: SidebarItem) => {
        ...
        if (item.children) {
          ...
          children.forEach((child: SidebarItem, index: number) => {
            if (!(child.permission && !this.checkPermissions(child.permission)))
              return;
  ...
  public getScheduleHours(): ScheduleLessonHour[] {
    ...
    for (let i = 0; i < hours; i++) {
      ...
      if (les[i - 1]?.end) {
        ...
        let breakTime =
          this.SchoolBreaks.find((_) => _.beforeHour == i + 1)?.minutes ??
          this.SchoolSettings.breakTimeMinutes;
        ...
        start =
          this.SchoolSettings.startTime[0] +
          ':' +
          this.addZeros(this.SchoolSettings.startTime[1], 2);
  ...
  public getFirstDayOfWeek(week: number): Date {
    ...
    monday.setDate(
      monday.getDate() + week * 7 + (monday.getDay() == 0 ? -6 : 0)
    );
  ...
  public getTimetableDay(date: number, month: number, year: number): Lesson[] {
    ...
    let lessons: Lesson[] = [];
    this.lessons?.[dat.getDay() == 0 ? 6 : dat.getDay() - 1]?.forEach(
      (les: Lesson[]) => {
        for (let i = 0; i < les.length; i++) {
          if (
            les[i].type == 0 ||
            (les[i].subject == -1 && les[i].teacher == -1) ||
            (les[i].type == 1 && dat.getWeek() % 2) ||
            (les[i].type == 2 && !(dat.getWeek() % 2))
          ) {
            lessons.push(les[i]);
          }
        }
  ...
  public getLastStudyDay(date: Date = new Date()): Date {
    let day = date;
    if (
      this.getTimetableLessons()?.[day.getDay() == 0 ? 6 : day.getDay() - 1]
    ) {
      let checkhour =
      ...
      if (
        !checkhour ||
        (day.getHours() == parseInt(checkhour[0]) &&
          day.getMinutes() < parseInt(checkhour[1])) ||
        day.getHours() < parseInt(checkhour[0])
      ) {
  ...
  public refreshTimetable(): void {
    ...
    if (
      this.lessons == this.timetableLessonsLast &&
      week == this.timetableSelectedWeekLast
    ) {
      return;
    }
    ...
    this.lessons.forEach((_: Lesson[][], index: number) => {
      ...
      // Check holidays
      let holidays = this.filterHoliday(
        dda.getDate(),
        dda.getMonth(),
        dda.getFullYear()
      );
      ...
      if (!day.holiday) {
        _.forEach((lesson: Lesson[], index) => {
          ...
          for (let i = 0; i < lesson.length; i++) {
            let { ...subject } =
              (this.getSubject(lesson[i].subject) as Subject) ?? false;
            let { ...teacher } =
              (this.getTeacher(lesson[i].teacher) as Teacher) ?? false;
            let { ...room } = lesson[i].room
              ? (this.getRoom(lesson[i].room as number) as Room) ?? false
              : {};
            ...
            if (
              ...
              if (
                ...
                l.push({
                  ...
                  room: room as Room,
                  group: lesson[i].group,
                  class: lesson[i].class,
                });
              }
            ...
            if (
              index == _.length - 1 &&
              index <= this.getScheduleHours().length
            ) {
              for (
                let i = 0;
                i < this.getScheduleHours().length - 1 - index;
                i++
              ) {
                day.lessons.push([{ isEmpty: true }]);
              }
  ...
  public getAverageBySubject(subject: number): string {
    ...
    return Number(total / total_devide)
      .toFixed(2)
      .replace('.', ',');
  }
  ...
  public filterHoliday(
    ...
    let hol = this.holidays.filter(
      (holiday) => holiday.date?.day == day && holiday.date.month == month + 1
    );
  ...
  public compareLessons(
    ...
    day1.forEach((_: TTableLesson[]) => {
      if (!_[0]) return;
      if (
        day2.filter((__) => __[0].subject?.[2] == _[0]?.subject?.[2]).length ==
          0 &&
